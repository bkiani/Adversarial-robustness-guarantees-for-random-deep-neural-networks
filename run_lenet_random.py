...
import keras
import foolbox
from setup_model_img import FF_img_model, img_data_generator, simple_conv_model, le_net_model, le_net_cut_model
...
csv_name = 'norm_scaling_lenet_1.csv'
df = initialize_df()
df.to_csv('./csv_files/' + csv_name)

for image_size in range(16,57,8): 
	for distance_type, distance_name, all_attacks in zip(distance_objects, distances_names, attack_dict_list):
		...
		for model_i in range(n_models):
			...
			# model = le_net_model(image_size = image_size, num_channels = 3)
			model = le_net_model(image_size = image_size, num_channels = 3)
