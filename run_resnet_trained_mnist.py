...
csv_name = 'resnet20trained_noscaledmnist_1.csv'
df = initialize_df()
df.to_csv('./csv_files/' + csv_name)

# distance_objects = [distance_objects[0]]
# distances_names = [distances_names[0]]
# attack_dict_list = [attack_dict_list[0]]
	...
	for model_i in range(n_models):
		...
		while temp_acc < 0.53:
			...
			model.train(X_train, y_train,
                     ...
                     n_epochs = 1)
			y_train_temp = model.predict(X_train[:1000])
			temp_acc = accuracy_from_real(y_train_temp, y_train[:1000])
